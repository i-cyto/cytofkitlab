...
To install **cytofkitlab** package, start R and run the following codes on R console:
...
`Notes`: **cytofkitlab** GUI is dependent on XQuartz windowing system (X Windows) on Mac (OS X > 10.7). Install XQuartz from http://xquartz.macosforge.org.
...
![cytofkitlab GUI](cytofkitlab_GUI.png)
...
```{r}
## See documentation, this function uses the output of main cytofkit function as its input
?cytof_clusterMtrx
```
