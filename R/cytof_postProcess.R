...
#' 
#' @param to_add The data matrix to add/append as columns/channels.
...
cytof_addToFCS <- function(
    ...
    if (!dir.exists(analyzedFCSdir)) {
        dir.create(analyzedFCSdir, recursive = TRUE)
    }
    
    # add some randomness cluster_cols
    for (i in cluster_cols){
        to_add[, i] <- to_add[, i] +
            asin(runif(nrow(to_add), -1, 1))/pi/2
    }

    # split sample_cellno key
    sample <- sub("_[0-9.]+$", "", rownames(to_add))
    sample_key <- factor(sample, levels = unique(sample))
    sample <- levels(sample_key)
    cellno_key <- as.integer(gsub(".+_([0-9]+$)", "\\1", rownames(to_add)))
    # 
    for (i in 1:length(sample)) {
        ...
        # skip if cannot find the file
        if(!file.exists(fn)){
            warning("Cannot find raw FCS file: ", fn)
            next()
        }
        cat("Processing file:", basename(fn), "\n")
        # identify rows to extract from the result
        to_add_i <- to_add[sample[i] == sample_key, ]
        cellno_i <- cellno_key[sample[i] == sample_key]
        # read the FCS file
        fcs <- cytof_readFCS(fn, comp = FALSE, verbose = FALSE)
        # extract the subset of cells
        fcs <- fcs[cellno_i, ]
        # update the expression value
        out_frame <- flowCore::fr_append_cols(fcs, to_add_i)
        ...
        suppressWarnings(write.FCS(out_frame, file.path(
            analyzedFCSdir, sprintf("ck_%s.fcs", sample[i]))))
    }
} 
